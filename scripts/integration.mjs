...
const { values } = parseArgs({ options: {
  ...
  aws: { type: 'boolean' }, 'table-prefix': { type: 'string' },
}});
const env = { ...process.env };
for (const [arg, name] of Object.entries({ endpoint: 'DYNAMODB_ENDPOINT', region: 'AWS_REGION', profile: 'AWS_PROFILE', 'table-prefix': 'DYNAMODB_TABLE_PREFIX' })) {
